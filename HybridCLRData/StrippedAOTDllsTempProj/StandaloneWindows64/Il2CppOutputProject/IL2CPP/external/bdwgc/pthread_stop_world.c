...
#ifdef HOST_ANDROID
  GC_INNER void GC_block_android_signal_catcher_signals(void)
  {
    sigset_t sys_signals;
    sigemptyset(&sys_signals);
    if (sigaddset(&sys_signals, SIGQUIT) != 0
          || sigaddset(&sys_signals, SIGUSR1) != 0)
      ABORT("sigaddset failed");
    if (pthread_sigmask(SIG_BLOCK, NULL, &sys_signals) != 0)
      ABORT("pthread_sigmask failed");
  }
#endif /* HOST_ANDROID */
...
#ifdef HOST_ANDROID
STATIC GC_bool GC_may_altstack(GC_thread me)
{
  ptr_t approx_sp, hi;
  stack_t stack;

  approx_sp = GC_approx_sp();
  hi = (me -> flags & MAIN_THREAD) == 0 ? me -> stack_end : GC_stackbottom;

  if ((word)hi HOTTER_THAN (word)approx_sp)
    return TRUE;

  if (sigaltstack(NULL, &stack) != 0)
    ABORT("sigaltstack failed");

  if (stack.ss_flags != 0 && stack.ss_flags != SS_DISABLE)
    return TRUE;
...
# ifdef HOST_ANDROID
    if (GC_retry_signals && GC_may_altstack(me)) {
...
#             endif
              if (is_retry && AO_load(&p->stop_info.last_stop_count) == GC_stop_count)
...
#           ifdef GC_ENABLE_SUSPEND_THREAD
              if (p -> suspended_ext) continue;
#           endif
            if (is_retry && AO_load(&p->stop_info.last_stop_count)
...
#   endif
#   ifdef HOST_ANDROID
      GC_block_android_signal_catcher_signals();
#   endif
# endif /* !GC_OPENBSD_UTHREADS && !NACL */
}
