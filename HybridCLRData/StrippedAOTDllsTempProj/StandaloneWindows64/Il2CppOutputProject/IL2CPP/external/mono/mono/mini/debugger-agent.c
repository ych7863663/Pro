...
gboolean vm_start_event_sent, vm_death_event_sent, disconnected;
...
MonoNativeThreadId debugger_thread_id;
...
MonoDebuggerAttachFunc attach_func;
MONO_API void
...
#if IL2CPP_WEIXINMINIGAME_DEBUGGER
    res = debugger_web_send(&buf, len, command_set, command);
#else
...
#if IL2CPP_WEIXINMINIGAME_DEBUGGER
    res = debugger_web_send(&buf, len, -1, -1);
#else
    res = transport_send (buf.buf, len);
    buffer_free (&buf);
#endif
...
process_event (EventKind event, gpointer arg, gint32 il_offset, MonoContext *ctx, GSList *events, int suspend_policy)
{
	...
	if (!send_success) {
#if IL2CPP_WEIXINMINIGAME_DEBUGGER
        PRINT_DEBUG_MSG (1, "[%p] Cache and late Sent %d events %s(%d), suspend=%d.\n", (gpointer) (gsize) mono_native_thread_id_get (), nevents, event_to_string (event), ecount, suspend_policy);
#else
...
{
    debugger_agent_web_main_loop_entry();
}
#else
/*
 * debugger_thread:
